...
function toNumber(value, def) {
  let result;
  result = Number(value);
  if (Number.isNaN(Number(value)) === true || Number(value) === 0) result = def;
...
function isPowerOfTwo(num) {
  let result;
  if (Number.isInteger(Math.log2(num))) result = 1;
  else result = 0;
...
function getSumOfNumbers(x1, x2, x3) {
  let result;
  if (Number.isInteger(x1 + x2 + x3) === true) result = x1 + x2 + x3;
  else result = (x1 + x2 + x3).toFixed(1);
  return result;
}
